...
internal static class RoslynGeneratedNamesParser
{
    ...
    internal static bool TryParseSourceMethodNameFromGeneratedName(string generatedName, RoslynGeneratedNameKind requiredKind, [NotNullWhen(true)] out string? methodName)
    {
    ...
    internal static bool TryParseLocalFunctionName(string generatedName, [NotNullWhen(true)] out string? localFunctionName)
    {
        ...
        if (!TryParseGeneratedName(generatedName, out var kind, out _, out int closeBracketOffset) || kind != RoslynGeneratedNameKind.LocalFunction)
        {
            return false;
        }
